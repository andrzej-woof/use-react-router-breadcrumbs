...
const components = {
  ...
  BreadcrumbRouteTest: ({
    ...
    match: { route?: { arbitraryProp: string } };
  }) => <span>{match.route?.arbitraryProp}</span>,

  BreadcrumbNavLinkTest: ({ match }: { match: { pathname: string } }) => (
    <a href={match.pathname}>Link</a>
  ),

  BreadcrumbLocationTest: ({ location }: BreadcrumbComponentProps) => (
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
  ...
  Layout: React.memo(({ children }: { children?: React.ReactNode }) => (
    <div>{children}</div>
  )),
};
...
const renderer = ({
  ...
  const wrapper = render(
    <MemoryRouter initialIndex={0} initialEntries={[{ pathname, state }]}>
      <Breadcrumbs
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        useBreadcrumbs={useBreadcrumbsHook}
...
describe("use-react-router-breadcrumbs", () => {
  describe("Valid routes", () => {
    it("Should render breadcrumb components as expected", () => {
      ...
      expect(
        getByTextContent("Home / One / TWO / 3 / Link / Any")
      ).toBeTruthy();
  ...
  describe("Route order", () => {
    ...
    it("Should match the correct breadcrumb in route if they have the same score", () => {
      ...
      expect(
        getByTextContent("Home / User / First Create / World")
      ).toBeTruthy();
    });
  });
  ...
  describe("When extending react-router config", () => {
    ...
    it("Should support nested routes", () => {
      ...
      expect(
        getByTextContent("Home / One / TwoCustom / ThreeCustom")
      ).toBeTruthy();
    });
    ...
    describe("defaultFormatter", () => {
      it("should be used if a breadcrumb is not provided for a specific path", () => {
        ...
        renderer({
          ...
          routes,
          options: {
            defaultFormatter: (breadcrumb: string) =>
              breadcrumb.replace(/two/g, "changed"),
          },
        });

        expect(
          getByTextContent("Home / One / changed / three_four")
        ).toBeTruthy();
      });
    });
  });
